...
export function MyPage({ onModalStateChange }: MyPageTabProps) {
    const router = useRouter();
    const [showLogoutModal, setShowLogoutModal] = useState(false);
    const [showAdminTab, setShowAdminTab] = useState(false);
    const [showDataManagementTab, setShowDataManagementTab] = useState(false);
    const [isClosingAdminTab, setIsClosingAdminTab] = useState(false);
    const [isClosingDataManagementTab, setIsClosingDataManagementTab] = useState(false);
    ...
    return (
        <>
                        ...
                        <CardContent className="p-6">
                            <div className="flex items-center space-x-4">
                                <div className="w-16 h-16 bg-white rounded-full flex items-center justify-center shadow-sm">
                                    <Image src="/logo.svg" alt="프로필" width={32} height={32} />
                                </div>
                ...
                <div className="fixed inset-0 bg-black/30 backdrop-blur-sm z-50">
                    <div className={`absolute top-0 right-0 h-full w-full max-w-md bg-white shadow-2xl transform transition-transform duration-300 slide-in-right ease-in-out ${
                        ...
                        <div className="flex items-center justify-between p-4 border-b border-gray-200">
                            <h2 className="text-lg font-semibold text-gray-900 mt-1">데이터 업로드</h2>
                            <Button
                                onClick={() => {
            ...
            {/* 데이터 관리 탭 슬라이드 오버레이 */}
            {(showDataManagementTab || isClosingDataManagementTab) && (
                <div className="fixed inset-0 bg-black/30 backdrop-blur-sm z-50">
                    <div className={`absolute top-0 right-0 h-full w-full max-w-md bg-white shadow-2xl transform transition-transform duration-300 slide-in-right ease-in-out ${
                        showDataManagementTab && !isClosingDataManagementTab ? 'translate-x-0' : 'translate-x-full'
                    }`}>
                        <div className="flex items-center justify-between p-4 border-b border-gray-200">
                            <h2 className="text-lg font-semibold text-gray-900 mt-1">데이터 관리</h2>
                            <Button
                                onClick={() => {
                                    setIsClosingDataManagementTab(true);
                                    setTimeout(() => {
                                        setShowDataManagementTab(false);
                                        setIsClosingDataManagementTab(false);
                                        onModalStateChange?.(false);
                                    }, 300);
                                }}
                                variant="ghost"
