...
interface BundleProcedureProps {
...
export default function BundleProcedure({ displayProduct, existingBundleInfo }: BundleProcedureProps) {
    ...
    return (
        <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                    <label className="text-xs text-gray-500">번들 총 원가</label>
                    <p className="text-sm font-medium text-gray-900">
            ...
            {/* 번들 상품인 경우 element 상세 정보 표시 */}
            {displayProduct.package_type === '번들' && displayProduct.bundle_id && existingBundleInfo && 'elements' in existingBundleInfo && (
                <div className="mt-4">
                    <ProcedureAccordion 
                        title="번들 상세 정보" 
                        subtitle={`번들 ID: ${displayProduct.bundle_id}`}
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <p className="text-sm font-medium text-gray-900">
                                        {(existingBundleInfo as BundleInfo).name || '이름 없음'}
                                    </p>
                                </div>
                                <div>
                                    <label className="text-xs text-gray-500">총 시술 수</label>
                                    <p className="text-sm font-medium text-gray-900">
                                        {existingBundleInfo.elements?.length || 0}개
                                    </p>
                                </div>
                                <div>
                                    <label className="text-xs text-gray-500">배포 상태</label>
                                    <p className="text-sm font-medium text-gray-900">
                                        {'release' in existingBundleInfo ? (existingBundleInfo.release === 1 ? '배포됨' : '미배포') : '정보 없음'}
                                    </p>
                                </div>
                            </div>
                            
                            {/* 번들 설명 */}
                            {(existingBundleInfo as BundleInfo).description && (
                                <div>
                            ...
                            {/* 포함된 시술들 상세 정보 */}
                            {existingBundleInfo.elements && existingBundleInfo.elements.length > 0 && (
                                <div>
                                    <label className="text-xs text-gray-500 mb-2 block">포함된 시술들 상세 정보</label>
                                    <div className="space-y-3">
                                        {existingBundleInfo.elements.map((element, index: number) => {
                                            // 타입 안전성을 위해 타입 가드 사용
                                            ...
                                            const elementDetailAsElement = elementDetail as Element;
