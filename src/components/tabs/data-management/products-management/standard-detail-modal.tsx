...
import { ProductDetailResponse, getStandardProductDetail, updateStandardProduct, StandardProductUpdateRequest} from '@/api/products-api';
...
export default function StandardDetailModal({ product, isOpen, onClose, onRefresh }: StandardDetailModalProps) {
    ...
    useEffect(() => {
        ...
        if (isOpen && product && product.id) {
            ...
            getStandardProductDetail(product.id)
                .then((detailData) => {
    ...
    }, [isOpen, product, fetchExistingBundleInfo]);
    ...
    const handleSave = async () => {
        ...
        try {
            // API 호출로 상품 정보 수정
            const { info_standard, ...restEditData } = editData;
            
            // API 요청 데이터 구성
            const apiData: StandardProductUpdateRequest = { ...restEditData } as StandardProductUpdateRequest;
            
            // info_standard 객체를 개별 필드로 분해
            if (info_standard) {
                apiData.product_standard_name = info_standard.name;
                apiData.product_standard_description = info_standard.description;
                apiData.precautions = info_standard.precautions;
            }
            
            const updatedProduct = await updateStandardProduct(displayProduct.id, apiData);
            
            // 수정된 데이터로 상태 업데이트
            setDetailProduct(updatedProduct);
            
            // 시술 정보나 상품 정보가 변경된 경우, 상세 정보를 다시 가져와서 완전한 데이터로 업데이트
            if (editData.element_id || editData.bundle_id || editData.custom_id || editData.sequence_id || editData.info_standard) {
                try {
                    const fullProductDataResponse = await getStandardProductDetail(displayProduct.id);
